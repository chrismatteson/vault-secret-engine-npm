...
func (b *backend) Client(ctx context.Context, s logical.Storage) (*npmgo.Client, error) {
	b.lock.RLock()
	...
	b.client, err = npmgo.NewClient(connConfig.URI, connConfig.Username, connConfig.Password)
	if err != nil {
		return nil, err
	}
...
const backendHelp = `
The NPM backend dynamically generates NPM access tokens.
